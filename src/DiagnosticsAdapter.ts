...
export type WorkerAccessor = (...uris: monaco.Uri[]) => Promise<TurtleLangWorker>;
...
export default class DiagnosticsAdapter {
  ...
  private async validate(resource: monaco.Uri): Promise<void> {
    ...
    model && monaco.editor.setModelMarkers(model, 'turtle', errorMarkers.map(toDiagnostics));

  }
}
function toDiagnostics(error: ITurtleLangError): monaco.editor.IMarkerData {
  return {
    ...error,
